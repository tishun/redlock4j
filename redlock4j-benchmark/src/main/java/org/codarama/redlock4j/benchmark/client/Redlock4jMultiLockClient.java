...
/**
 * MultiLock client implementation using redlock4j with Lettuce driver.
 */
public class Redlock4jMultiLockClient extends AbstractMultiLockClient {

    private static final String IMPLEMENTATION_TYPE = "redlock4j-multilock";
    ...
    public void initialize(BenchmarkConfiguration config, RedisClusterManager clusterManager) {
        ...
        try {
            ...
            logger.info("Redlock4j MultiLock client initialized with {} nodes, healthy={}",
                    nodes.size(), healthy);
        } catch (Exception e) {
            logger.error("Failed to initialize Redlock4j MultiLock client: {}", e.getMessage());
            healthy = false;
        }
    }
    ...
    public void close() {
        if (redlockManager != null) {
            ...
            logger.info("Redlock4j MultiLock client closed");
        }
        healthy = false;
    }
}
