...
/**
 * Distributed lock client using redlock4j with 3-node Redlock algorithm.
 */
public class Redlock4jDistributedLockClient extends AbstractDistributedLockClient {

    private static final String IMPLEMENTATION_TYPE = "redlock4j-3node";
    ...
    public void initialize(BenchmarkConfiguration config, RedisClusterManager clusterManager) {
        ...
        try {
            ...
            logger.info("Redlock4j distributed lock client initialized with {} nodes, healthy={}",
                    nodes.size(), healthy);
        } catch (Exception e) {
            logger.error("Failed to initialize Redlock4j client: {}", e.getMessage());
            healthy = false;
        }
    }
    ...
    public void close() {
        if (redlockManager != null) {
            ...
            logger.info("Redlock4j distributed lock client closed");
        }
        healthy = false;
    }
}
