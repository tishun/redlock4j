...
/**
 * ReadWriteLock client using Redisson's RReadWriteLock.
 */
public class RedissonReadWriteLockClient extends AbstractReadWriteLockClient {
    ...
    public void initialize(BenchmarkConfiguration config, RedisClusterManager clusterManager) {
        ...
        try {
            ...
            healthy = true;
            logger.info("Redisson ReadWriteLock client initialized, connected to {}", primaryNode.getAddress());
        } catch (Exception e) {
            logger.error("Failed to initialize Redisson ReadWriteLock client: {}", e.getMessage());
            healthy = false;
        }
    }
    ...
    public ReadWriteLock getReadWriteLock(String resourceName) {
        ...
        return redissonClient.getReadWriteLock(resourceName);
    }
    ...
    public void close() {
        if (redissonClient != null && !redissonClient.isShutdown()) {
            ...
            logger.info("Redisson ReadWriteLock client closed");
        }
        healthy = false;
    }
}
