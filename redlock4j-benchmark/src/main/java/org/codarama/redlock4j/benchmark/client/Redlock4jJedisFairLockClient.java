...
/**
 * Fair lock client implementation using redlock4j with Jedis driver.
 */
public class Redlock4jJedisFairLockClient extends AbstractFairLockClient {

    private static final String IMPLEMENTATION_TYPE = "redlock4j-jedis";
    ...
    public void initialize(BenchmarkConfiguration config, RedisClusterManager clusterManager) {
        ...
        try {
            redlockManager = RedlockManager.withJedis(redlockConfig);
            healthy = redlockManager.isHealthy();
            logger.info("Redlock4j (Jedis) client initialized with {} nodes, healthy={}", 
                    nodes.size(), healthy);
        } catch (Exception e) {
            logger.error("Failed to initialize Redlock4j (Jedis) client: {}", e.getMessage());
            healthy = false;
        }
    }
    ...
    public void close() {
        if (redlockManager != null) {
            ...
            logger.info("Redlock4j (Jedis) client closed");
        }
        healthy = false;
    }
}
